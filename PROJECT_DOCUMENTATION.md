...
- Кнопка фильтра (иконка) открывает поп-ап выбора серий:
  - Список серий формируется из текущего расписания, отсортирован по алфавиту, в одну колонку с чекбоксами
  - Чекбоксы при открытии пустые, если фильтр не активен; если фильтр активен — отмечены серии, которые сейчас отображаются
  - В конце списка чекбокс "Все серии" включает/выключает все пункты
  - Кнопка OK: если ни один пункт не выбран, показывает предупреждение "Не выбрана ни одна серия"; иначе применяет фильтр и закрывает поп-ап
  - Кнопка "Сброс": закрывает поп-ап, снимает фильтр и показывает все серии
  - Крестик в заголовке: закрывает поп-ап без перерисовки страницы
