...
6. Нормализует даты (в CSV теперь `DD.MM.YYYY`) к короткому виду `DD.MM.YY` для вывода и дедупликации дней
7. Группирует по дням через `groupBy()` с ключом `${date}_${day}`
8. Рендерит колонки дней с заголовками и строками расписания
...
- Кнопка фильтра (иконка) открывает поп-ап:
  - Вкладки: "Фильтр по сериям" (список серий, алфавит, чекбоксы) и "Фильтр по дням" (список дат по возрастанию в формате `DD.MM.YY, <день недели>`)
  - Чекбоксы "Все серии" / "Все дни" — включают/выключают все
  ...
  - Кнопка "Сброс": снимает фильтр, показывает все
