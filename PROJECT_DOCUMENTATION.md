...
- Кнопка фильтра (иконка) открывает поп-ап:
  - Вкладки: "Серии" (список серий, алфавит), "Дни" (даты по возрастанию, `DD.MM.YY, <день недели>`), "Трассы" (места), "Комментаторы" (Commentator1/2 + "Оригинальная дорожка" для пустых)
  ...
  - Правила OK:
    ...
    - если выбраны только трассы → показываются строки выбранных трасс
